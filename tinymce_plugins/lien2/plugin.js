 ...
 */
    ...
    const register$1 = editor => {
      editor.addCommand('mceMaxLien', Dialog(editor).open);
    ...
    const register = editor => {
      editor.ui.registry.addToggleButton('maxLien', {
        icon: 'link',
        tooltip: 'Insert link',
      ...
      editor.ui.registry.addMenuItem('maxLien', {
        icon: 'link',
        text: 'Lien...',
        onAction: Dialog(editor).open
      });
      editor.ui.registry.addContextMenu('maxLien', { update: element => isFigure(element) || isImage(element) && !isPlaceholderImage(element) ? ['image'] : [] });
    };

    const loadPlugin = () => {
      tinymce.PluginManager.add('maxLien', editor => {
